...
- **The player never blocks the entry**: `getPlayer()` (10 s) starts in parallel with init's tail. Failure / hang = **guest mode** (`guest_mode`) + the background retry **5 × every 15 s** (`guest_recovered`). `identity.ready()` = the SDK is up, not "the player is known".
...
6. A write answered `true` while another is in flight can still fail with the final turn — the donor's own trade-off against retry spam.
