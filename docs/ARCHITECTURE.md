...
`game-core/pixi` (source `src/pixi/`, bundle `dist/pixi/`, art `assets/pixi-ui/`) contains `LevelMapView`, `HudView`, `UiButton`, `ModalWindow` with `ResultWindowView` / `LivesWindowView` / `ShopWindowView` / `SettingsWindowView` / `NoAdsWindowView` / `StarterPackWindowView`, `loadReadyUiAssets` and the theme (Theme System V1: `resolveTheme` merges a game's tokens in depth over `DEFAULT_READY_UI_THEME`; panels, semantic button roles, the × close control, badges, cards and tab shells are drawn from those tokens with Pixi Graphics in `src/pixi/skin.ts` — V1.1 adds the glossy Bubble language (`BUBBLE_READY_UI_THEME`: a top gloss band, a flat lip, a soft layered shadow on every surface, a light inner card as the second layer) as tokens of the same system — while icons, illustrations and other complex art stay textures and are never tinted); geometry, assets and entrances are taken 1:1 from the Trail Arrow prefabs. The kit never imports `OfferRuntime`: `StarterPackWindowView` only receives data (`show` params, `setTimer`, `setBuyEnabled`) from the host's offer adapter. The views are PixiJS containers laid out in viewport px over a contain-fit design box; every tap is a `ButtonController`, every modal a `WindowController`, every animation a `MotionRuntime` tween or sequence in a view-owned scope, so `core.cancelAll()` settles the whole interface and no business callback (level selection, NEXT, BUY) can fire from a cancelled press or a force-hidden window — they run only as settled taps and `close()` continuations. The standalone showcase (`npm run showcase`) proves the layer without any game attached.
