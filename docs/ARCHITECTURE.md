...
`YandexPlatform` (v0.8-B1) is a 1:1 port of Trail Arrow's production Yandex integration: 20 s init + one retry + a 30 s dead-SDK memory, a player that never blocks the entry (guest mode, 5 × 15 s background retry), a cloud read of 8 s × 3 that REJECTS on failure and for a guest, one throttled `setData` in flight (≥ 3 s) that always sends the whole confirmed object with the patch on top (the SDK replaces the player's object; one full `getData()` before the first write of a session, the confirmed object moves only on success — `storage.set` / `clear` stay PATCHes), the rewarded answer only when the show ends, the local rewarded-availability latch, audio / gameplay restored in `finally`, the 2 s / 120 s ad watchdog, every payments timeout, `restoreGrant: 'after-consume'`. The game's ECS components, sound and analytics calls are injected hooks; the mirror, rollback guard, `save_seq`, identity adoption, the granted registry, `registerShown` and every retry schedule stay in the host. `src/platform/support/` (`withTimeout`, `createAdWatchdog`) is adapter support — timers and `document` live only there and are injected. `scripts/check-platform-build.mjs` enforces the boundary on the built files. Spec: `docs/superpowers/specs/2026-09-17-platform-yandex-v0.8-b1-design.md`. CleverApps, autodetect and the JWT resolver are later slices.

## Save Gate

`SaveGate` (`src/save/`, root entry, V1) is the load-before-write rule that both clean integrations (SoliPix, Gorodki) wrote by hand, over `PlatformStorage` — no second storage abstraction. `new SaveGate({ storage: platform.storage, profile })` reads `profile.id` and `profile.save`; `load()` (once, never rejects) issues one `storage.get` per read group — `save.groups` (optional, a partition of `save.keys`; absent = one group) — and answers `{ values, groups: [{ keys, status, error? }], core }`. A group is `loaded`, `missing` (the read succeeded and nothing is stored — a new player starts from defaults and may save them) or `failed` (the read rejected or answered a non-object — the gate never writes those keys, so a default can never overwrite a save that could not be read). The phases are `idle → loading → loaded → open`: writes answer `not_open` until the host has applied the loaded values and called `open()`; `write(patch)` refuses a key outside `save.keys` (`unknown_key`) or in a failed group (`read_failed`) without touching the storage, otherwise it is one `storage.set` answered `ok` / `storage_refused` / `storage_rejected` — never a throw. Core-owned state is one object under `<profile.id>.core` (`readCore` / `writeCore(record, value)`), read with the first group (one read, one failure domain), never merged into a game key; the profile validator rejects a game key equal to it. Out of scope by design: the game's save format and migrations, the local mirror, cloud / local and guest / player merges, retries, timers. Correctness of a multi-key gate relies on the `PlatformStorage.set` patch contract; the Yandex adapter keeps it over the replacing `setData` by writing the whole confirmed object (Yandex Platform Storage safe PATCH V1).

## Soft Currency Wallet

`SoftCurrencyWallet` (`src/economy/`, root entry, V1) keeps the one soft currency of a game whose profile says `economy.softCurrency.owner = 'core'` (a gameplay without coins of its own — Gorodki). A gameplay-owned currency (SoliPix, Trail Arrow) keeps its own balance and gets no wallet (the constructor refuses it). `new SoftCurrencyWallet({ gate, profile })` takes the game's `SaveGate` and stores one record in its Core namespace: `<profile.id>.core` → `{ wallet: { v: 1, balance, rewardedLevels } }` — never a game key, no second save system. `load()` (once, never rejects) awaits `gate.load()`: Core read `missing`, or a Core record without `wallet` → `ready` at `startBalance`; a valid V1 record → `ready` at the stored balance; a `failed` Core read → `unavailable` / `read_failed`; a stored record that is not a V1 wallet (another `v`, a negative or fractional balance …) → `unavailable` / `invalid_record`. Unavailable means the balance is UNKNOWN: `balance` is `null` (never 0, never `startBalance`) and every change is refused for the gate's lifetime, so a default can never overwrite a balance that could not be read.

Changes — `grant(amount, { source?, item? })`, `trySpend(amount, context?)` (`insufficient_funds`), `applyLevelResult(result)`, `save()` — are accepted only while the wallet is `ready` AND the gate is `open` (`writable`); otherwise `not_loaded` / `unavailable` / `not_open`. An amount must be an integer ≥ 0 (0 is a no-op; negative, fractional, NaN, non-numbers and a balance past the safe-integer range → `invalid_amount`). An accepted change is synchronous in memory, then ONE `gate.writeCore('wallet', record)` of the whole record (`result.saved`, never rejects); a failed write keeps the in-memory balance and is carried by the next write or `save()`. Results are `{ ok, reason, balance, saved }` — never a throw, so `wallet.grant` fits PurchaseRuntime's synchronous `grant` hook (coin packs) and OfferRuntime rewards (`OfferReward.id === wallet.currency`) without new orchestration. `onChange(listener)` reports every balance change `{ currency, kind: grant | spend | level_reward, delta, balance, context? }` (none for a refusal, a 0 amount, a fail or the load); a throwing listener lands in `onListenerError` (default console.error), the change stands.

Level reward (Contract V1 `levelEnd`): a fail pays nothing. A win is `first` when the gameplay says `firstCompletion` AND the wallet has not paid this level yet → `profile.levelReward(result)`, and the level identity (`levelId`, else the level number) is recorded as paid in the persisted record — so neither a repeated identical levelEnd nor a later session pays a first completion twice; any other win is a `replay` → `profile.replayReward(result)`, 0 when the profile has none (no farming unless the game opts in). Core reads only `win`, `firstCompletion` and the level identity; the numbers are the profile's policies; a policy that throws or answers anything but an integer ≥ 0 → `invalid_reward`, nothing paid. Contract V1 has no run identity, so with `replayReward` opted in a duplicated levelEnd is paid as one more replay — the host must deliver one levelEnd per run. The result `{ kind, amount, balance }` is what the Ready UI takes as is: `HudView.setCoins(balance)`, `ResultWindowView.show({ …, rewardCoins: amount })`. Out of scope by design: a gameplay-owned currency, several currencies, prices, shops, purchase / offer orchestration, retries, timers.

## Lives Runtime

`LivesRuntime` (`src/lives/`, root entry, V1) keeps the lives of a game whose lives belong to Core — the generic semantics of Trail Arrow 0.1.31 (ClientLogicSystem / HeartsRefillSystem), none of its numbers. `new LivesRuntime({ gate, config: { maxLives, startLives?, regenSeconds, refundOnWin }, now? })` takes the game's `SaveGate` and stores one record in its Core namespace, beside the wallet: `<profile.id>.core` → `{ lives: { v: 1, lives, regenStart, attempt? } }` (`regenStart` = epoch ms the running regeneration period began, `null` at the cap; `attempt: true` while a paid attempt is open — V1.1, optional, absent = closed, so the version stays 1 and every earlier record reads unchanged; a non-boolean `attempt` is an invalid record). The config is the host's (Trail Arrow: 5 / 1800 / true; `startLives` defaults to `maxLives`; an unknown key is refused); the Production Profile is unchanged — `ui.lives` stays the HUD boolean and a later composer may connect the two. `now` defaults to `Date.now`; a host with a server-anchored clock passes it. `load()` (once, never rejects) follows the wallet's rule: Core read `missing` or no `lives` record → `ready` at `startLives`; a valid V1 record → `ready` (a stored count above a lowered cap is clamped); a `failed` read → `unavailable` / `read_failed`; another version or shape → `unavailable` / `invalid_record`. Unavailable means the count is UNKNOWN: `lives` is `null`, every change — spend, grant, refill, save — is refused and nothing is written, so neither a default nor a free refill can overwrite lives that could not be read; the host decides whether a level may be played without lives meanwhile.

Regeneration has no timer: every call applies the clock to the stored stamp — whole elapsed periods become lives (up to the cap) and the stamp keeps the remainder, so a closed game (any offline span) regenerates exactly like a running one; at the cap no period runs and the next spend starts a full one; a missing stamp or a clock behind the stamp (moved back, or a save from a clock that ran ahead) re-anchors the period at now — never a life taken (the donor's `floor` of a negative span did take one), never a timer frozen until the old time; a non-finite clock changes nothing. `snapshot()` applies time in memory (`{ status, problem, lives, maxLives, full, writable, canStart, attemptOpen, nextLifeAt, nextLifeInMs }`), `tick()` — from the host's existing HUD second — also writes what time changed. Regenerated lives are reported to listeners once (`regen`).

Attempts: `startAttempt()` pays one life at the moment the game chooses (Trail Arrow: the level entry, after the level loaded; other games: the first move) — 0 lives → `no_lives`, the host shows `LivesWindowView`; one attempt runs at a time, so a repeated start pays nothing, and a restart is `endAttempt()` + `startAttempt()` (a new payment). `endAttempt('win' | 'fail' | 'exit')` ends it once (a repeated end → `no_attempt`); a win returns the life under `refundOnWin`, up to the cap; a fail or an exit returns nothing. One logical attempt = one life (V1.1): the open attempt is durable in the same record — the spend and the open flag are one write, the end (with a win's refund) and the close are one write, so a crash leaves either the attempt open with its life paid or closed with its refund. After a reload the stored open attempt belongs to no session yet: `resumeAttempt()` — called by the host when the gameplay RESTORED its unfinished run (SoliPix `onLevelChanged({ restored: true })`) — continues it without a spend and without a write (idempotent; `no_attempt` when none is open, e.g. its write never landed — the host then starts a new one); `startAttempt()` instead treats it as abandoned (closed, no refund) and pays the new run in one write; `endAttempt()` needs an attempt this session started or resumed. `snapshot().attemptOpen` is that session ownership. No attempt id: the gameplay's own restored / new signal is the identity, never the level number (a level is played many times); a run the gameplay only suspends (kept to be resumed, SoliPix exit to the map) is not ended. A write that failed is not durable: the reload sees the last durable state (a start that never landed → nothing to resume, one spend in total; an end that never landed → still resumable) and `save()` retries. `grant(amount, source?)` adds up to the cap (`delta` = what was really added) and is called only after a confirmed delivery — a watched rewarded ad, a delivered purchase, an offer reward — AdsRuntime / PurchaseRuntime stay the delivery owners and are not imported; `refill(source?)` sets the cap after the host took the price, `full` at the cap so nothing is charged. Changes need `ready` AND an open gate (`not_loaded` / `unavailable` / `not_open` / `disposed`), are synchronous in memory, then ONE `gate.writeCore('lives', record)` (`result.saved`, never rejects); a failed write keeps the memory and is carried by the next write or `save()`. `onChange` reports `{ kind: spend | refund | grant | refill | regen, delta, lives, maxLives, source? }`; `dispose()` drops the listeners and refuses changes. The snapshot is what the Ready UI takes: `hud.setMaxLives(maxLives)`, `hud.setLives(lives, formatTimer(Math.ceil(nextLifeInMs / 1000)))`, `LivesWindowView.show({ lives, maxLives, timerText, refillPrice, adOffer })` with the price and the ad offer from the host. Out of scope by design: prices, free-level rules, unlimited-lives time, analytics wiring, windows, several life kinds, server time.
